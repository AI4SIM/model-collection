...
class Plotter:
    ...
    def dispersion_plot(self, y: np.ndarray, y_hat: np.ndarray, plot_path=config.plots_path) -> None:
        ...
        err = np.sqrt((y - y_hat)**2)
        for i in range(len(bins)-1):
            idx = np.logical_and(y.flatten() >= bins[i], y.flatten() < bins[i+1])
            if np.all(idx):
    ...
    def histo(self, y: np.ndarray, y_hat: np.ndarray, plot_path=config.plots_path) -> None:
        ...
        ax.hist(y_hat.flatten(), bins=50, density=False, histtype="step", lw=3, label=self.label_predicted)
        ax.set_yscale("log")
    ...
    def histo2d(self, y: np.ndarray, y_hat: np.ndarray, plot_path=config.plots_path) -> None:
        ...
        h = ax.hist2d(y.flatten(), y_hat.flatten(), bins=100, norm=LogNorm(), cmap='Blues');
        plt.colorbar(h[3])
        ax.set_xlim(-50, 1200)
        ax.set_ylim(-50, 1200)
        ax.set_xlabel(self.label_target)
    ...
    def boxplot(self, y: np.ndarray, y_hat: np.ndarray, plot_path=config.plots_path) -> None:
        ...
        for i in range(y.shape[0]):
            flat_err.append(np.sqrt((y[i, :, :, :] - y_hat[i, :, :, :])**2).flatten())
