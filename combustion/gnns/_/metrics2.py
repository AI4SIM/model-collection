...
def flame_surface(batch_idx, y_hat, y, stage: str, grid_shape):

    def _flame_surface(tensor):
        # TODO unsatisfaying reshape of grid shape
        z, y, x = grid_shape
        tensor = tensor.reshape((x, y, z))
        tfs = torch.sum(tensor, axis=(-2, -1)).cpu()
