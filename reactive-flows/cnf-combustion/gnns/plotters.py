...
class Plotter:
    ...
    def dispersion_plot(self, y_val: np.ndarray, y_hat: np.ndarray) -> None:
        ...
        for i in range(len(bins) - 1):
            idx = np.logical_and(
                y_val.flatten() >= bins[i], y_val.flatten() < bins[i + 1]
            )
            if np.all(idx):
    ...
    def histo2d(self, y_val: np.ndarray, y_hat: np.ndarray) -> None:
        ...
        histo = ax.hist2d(
            y_val.flatten(), y_hat.flatten(), bins=100, norm=LogNorm(), cmap="Blues"
        )
    ...
    def boxplot(self, y_val: np.ndarray, y_hat: np.ndarray) -> None:
        ...
        for i in range(y_val.shape[0]):
            flat_err.append(
                np.sqrt((y_val[i, :, :, :] - y_hat[i, :, :, :]) ** 2).flatten()
            )
    ...
    def total_flame_surface(
        ...
        for i in range(gt_total_flame_surface.shape[0]):
            ...
            fig.add_trace(
                go.Scatter(
                    x=np.arange(gt_total_flame_surface.shape[1]),
    ...
    def cross_section(
        ...
        for i in range(y_hat.shape[0]):
            ...
            fig.add_trace(
                go.Contour(
                    z=sigma[zslice, :, :] - prediction[zslice, :, :],
                    zmin=-300 / norm_val,
                    zmax=300 / norm_val,
                    contours=dict(
                        showlines=False,
