...
class ThreeDCorrectionDataprocSyntheticData(ThreeDCorrectionDataproc):
    ...
    def download(self) -> xr.DataArray:
        ...
        sca_inputs = da.from_array(
            np.random.normal(mu, sigma, (100, 17)).astype("float32")
        )
        col_inputs = da.from_array(
            np.random.normal(mu, sigma, (100, 137, 27)).astype("float32")
        )
        hl_inputs = da.from_array(
            np.random.normal(mu, sigma, (100, 138, 2)).astype("float32")
        )
        pressure_hl = da.from_array(
            np.random.normal(mu, sigma, (100, 138, 1)).astype("float32")
        )
        inter_inputs = da.from_array(
            np.random.normal(mu, sigma, (100, 136, 1)).astype("float32")
        )
        ...
        flux_dn_sw = da.from_array(
            np.random.normal(mu, sigma, (100, 138)).astype("float32")
        )
        flux_up_sw = da.from_array(
            np.random.normal(mu, sigma, (100, 138)).astype("float32")
        )
        flux_dn_lw = da.from_array(
            np.random.normal(mu, sigma, (100, 138)).astype("float32")
        )
        flux_up_lw = da.from_array(
            np.random.normal(mu, sigma, (100, 138)).astype("float32")
        )
        ...
        xrds = xr.Dataset(
            data_vars=dict(
                sca_inputs=({"column": 100, "sca_variables": 17}, sca_inputs),
                col_inputs=(
                    {"column": 100, "level": 137, "col_variables": 17},
                    col_inputs,
                ),
                hl_inputs=(
                    {"column": 100, "half_level": 138, "hl_variable": 2},
                    hl_inputs,
                ),
                pressure_hl=(
                    {"column": 100, "half_level": 138, "p_variable": 1},
                    pressure_hl,
                ),
                inter_inputs=(
                    {"column": 100, "level_interface": 136, "inter_variable": 1},
                    inter_inputs,
                ),
                lat=({"column": 100}, lat),
                lon=({"column": 100}, lon),
